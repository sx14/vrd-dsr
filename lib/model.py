...
from lib.evaluation import eval_reall_at_N, eval_obj_img, eval_reall_at_N_hier, eval_reall_at_N_mul
...
def test_pre_net_hier(net, args):
    net.eval()
    time1 = time.time()
    res = {}
    rlp_labels_ours  = []
    ...
    rec_100_zs = eval_reall_at_N_hier(args.ds_name, 100, res, use_zero_shot = True)
    print 'CLS TEST r50:%f, r50_zs:%f, r100:%f, r100_zs:%f'% (rec_50, rec_50_zs, rec_100, rec_100_zs)
    time2 = time.time()
    print "TEST Time:%s" % (time.strftime('%H:%M:%S', time.gmtime(int(time2 - time1))))
    return rec_50, rec_50_zs, rec_100, rec_100_zs
...
def test_rel_net_mul(net, args):
    ...
    rec_100 = eval_reall_at_N_mul(args.ds_name, 100, res, use_zero_shot = False)
    rec_100_zs = eval_reall_at_N_mul(args.ds_name, 100, res, use_zero_shot = True)
    print 'CLS OBJ TEST POS:%f, LOC:%f, GT:%f, Precision:%f, Recall:%f'% (pos_num, loc_num, gt_num, pos_num/(pos_num+loc_num), pos_num/gt_num)
    print 'CLS REL TEST r50:%f, r50_zs:%f, r100:%f, r100_zs:%f'% (rec_50, rec_50_zs, rec_100, rec_100_zs)
    time2 = time.time()
    print "TEST Time:%s" % (time.strftime('%H:%M:%S', time.gmtime(int(time2 - time1))))
